...
class Simulation(ASIF, Control, Dynamics):
    ...
    def sim(self):
        ...
        for i in range(1, total_steps):
            ...
            # If safety check on, use safety filter to modify u_des
            if self.safety_flag:
                if self.bCBF_QP:
                    u, boolean, sdt = self.vanilla_bCBF(x_curr, u_des)
                elif self.regFun:
                    u, boolean, sdt = self.vanilla_blending(x_curr, u_des)
                elif self.OI_QP_flag:
                    u, boolean, sdt = self.OI_QP(x_curr, u_des)
